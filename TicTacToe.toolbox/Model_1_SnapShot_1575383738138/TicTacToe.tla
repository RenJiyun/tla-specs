...
RowInvariant ==
    /\ \A row \in rows: 
            LET rowVal == [i \in row |-> board[i]]                    \* 定义一个函数
            IN rowVal[row] \in Seq({0, nil}) \/ rowVal[row] \in Seq({1, nil})
    
\* 列不变式  
ColInvariant == 
    /\ \A col \in cols: 
            LET colVal == [i \in col |-> board[i]]
            IN colVal[col] \in Seq({0, nil}) \/ colVal[col] \in Seq({1, nil})
...
DiagInvariant ==
    /\ \A diag \in diags: 
            ...
            IN diagVal[diag] \in Seq({0, nil}) \/ diagVal[diag] \in Seq({1, nil})
